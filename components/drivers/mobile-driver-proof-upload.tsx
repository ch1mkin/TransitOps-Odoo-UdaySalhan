...
export function MobileDriverProofUpload({
  ...
  const handleUpload = async () => {
    ...
    const result = await uploadDriverProofByToken(token, formData);
    setUploading(false);
      ...
      <div className="mx-auto max-w-md rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Driver identity upload</p>
          </div>
        </div>

        <div className="mt-6 space-y-2">
          <h1 className="text-lg font-semibold">Upload {documentType}</h1>
          <p className="text-sm text-muted-foreground">
            Take a clear photo of the document or choose one from your gallery. Return to the desktop screen to crop and finish.
          </p>
        </div>
        ...
        {done ? (
          <div className="mt-6 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-center">
            ...
            <p className="mt-1 text-xs text-emerald-700">
              Go back to your desktop to crop and complete the driver profile.
            </p>
          </div>
        ) : (
